"""
Run JailDAM demo with JailbreakV-Nano dataset.
Same architecture as demo.ipynb / run_figstep.py with:
  1. Fixed random seed for reproducibility.
  2. All 410 JailbreakV-Nano samples used for OOD evaluation.
"""
...
from UnsafeVLMDataset_jailbreakv_nano import main as main_unsafe_nano
...
if __name__ == "__main__":
    ...
    print("\n=== Loading datasets ===")
    unsafe_dataset, unsafe_dataloader = main_unsafe_nano(ds_model, ds_processor)
    safe_dataset,   safe_dataloader   = main_safe_mmvet(ds_model, ds_processor)
    print(f"JailbreakV-Nano unsafe : {len(unsafe_dataset)} samples")
    ...
    print(f"Train safe: {train_size}  |  Val safe: {val_size}  |  OOD unsafe (JailbreakV-Nano): {len(unsafe_dataset)} (ALL)")
    ...
    print(f"Unsafe samples (JailbreakV-Nano): {results['N_unsafe']}")
